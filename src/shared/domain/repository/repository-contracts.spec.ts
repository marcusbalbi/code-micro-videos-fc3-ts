...
describe("RepositoryContracts", () => {
  describe("SearchParams", () => {
    ...
    test("page props: filter", () => {
      ...
      arrange.forEach((i) => {
        expect(new SearchParams({ filter: i.filter as any }).filter).toEqual(
          i.expected
        );
      });
    });
  ...
  describe("SearchResult", () => {
    ...
    it("should set last page 1 when per_page is greater than total field", () => {
      ...
      expect(result.last_page).toEqual(1);
    });
    ...
    test("last_page prop when total is not a multiple of per_page", () => {
      ...
      expect(result.last_page).toEqual(6);
    });
  });
});
